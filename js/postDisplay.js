...
async function getPostData(i, postContent, postTags, postDate, postTime, userName) {
    if (i == 0) {
        ...
        db.collection("posts").orderBy("time","desc").startAfter(lastPost).limit(1).get().then(snap => {
            lastPost = snap.docs[snap.docs.length-1]
            snap.docChanges().forEach(docs => {

                // quillPlace.innerHTML = docs.doc.data().description;

                postTags.textContent = docs.doc.data().tags;
...
function createDiv(i) {
    ...
    getPostData(i, postContent, postTags, postDate, postTime, userName).then( function() {
